...
import com.texa.care.eco_driving.events.drivingevents.AccelerationsEvent;
import com.texa.care.eco_driving.events.scoreevents.AccelerationsScoreUpdateEvent;
...
public class AccelerationsScore extends BaseScore {
    public static final double ACC_THRESHOLD_G = 0.3d;
    ...
    public ScoreUpdateEvent createScoreUpdateEvent() {
        return new AccelerationsScoreUpdateEvent(this);
    }
    ...
    static {
        ...
        map.put(BaseScore.DrivingProfile.CHAUFFEUR, new Settings(1.3d, 5.3d));
        map.put(BaseScore.DrivingProfile.EXPERIENCED, new Settings(1.1d, 5.3d));
        map.put(BaseScore.DrivingProfile.NEWBIE, new Settings(0.9d, 5.3d));
    }
    ...
    public AccelerationsScore() {
        super("accelerations (progressive) score", 100);
    ...
    AccelerationsScore(String str, int i) {
        super(str, i);
        this.speed = SPEED_INVALID;
        this.snapshotStack = new CountersSnapshotStack(50, 20.0d);
        this.speed = SPEED_INVALID;
    ...
    public void update(List<CounterInfo> list) {
        ...
        if (shouldNotifyEvent(this.snapshotStack)) {
            ...
            AccelerationsEvent accelerationsEvent = new AccelerationsEvent(contextDate, this);
            Pair<Double, Double> pairEventCounts = eventCounts(this.snapshotStack);
            if (pairEventCounts != null) {
                accelerationsEvent.addExtra(Constants.COUNT, pairEventCounts.first);
                accelerationsEvent.addExtra(Constants.COUNT_FOR_EVENTS, pairEventCounts.second);
            }
            accelerationsEvent.acquiredPosition(getLocationProvider());
            addEvent(accelerationsEvent);
        }
    }
    ...
    private TupleSupport calcCount(List<CounterInfo> list) {
        ...
        Timber.d("ACCELERATION SCORE COUNT --> %s", Double.valueOf(d));
    ...
    private double countForEventNotification(double d) {
        double d2 = getSettings().EVENT_NOTIFICATION_COUNT;
        if (d == SPEED_INVALID) {
            return d2;
        }
        return Math.min(getSettings().EVENT_NOTIFICATION_COUNT + 2.0d, (getSettings().EVENT_NOTIFICATION_COUNT + 1.8d) - Math.log10(d));
    }
    ...
    private Pair<Double, Double> eventCounts(CountersSnapshotStack countersSnapshotStack) {
        ...
        Timber.d("ACCELERATION SCORE COUNT NORM --> %s / %s", Double.valueOf(d), Double.valueOf(dCountForEventNotification));
