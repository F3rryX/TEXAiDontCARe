...
public class TripEndInfoFactory {
    ...
    private static final List<Integer> SUPPORTED_BLOCK_ID = Collections.singletonList(Integer.valueOf(CareStructID.TRIP_END_INFO.getID()));
    public static final String TAG = "TripEndInfoFactory";

    public TripEndInfo create(byte[] bArr) {
        if (bArr.length < 5) {
            return null;
        }
    ...
    protected CareStructParser<TripEndInfo> getParserByBlockVersion(int i) {
        if (i == 1) {
            return new TripEndInfoParserV1();
        }
        return new TripEndInfoParserV2();
    }
    ...
    protected boolean isTripEndInfoBlock(int i) {
