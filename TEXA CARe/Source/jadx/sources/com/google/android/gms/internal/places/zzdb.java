...
class zzdb<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    public void zzab() {
        ...
        if (this.zzlr.isEmpty()) {
            ...
            mapUnmodifiableMap = Collections.unmodifiableMap(this.zzlr);
        }
        this.zzlr = mapUnmodifiableMap;
        if (this.zzlt.isEmpty()) {
            mapUnmodifiableMap2 = Collections.emptyMap();
        } else {
            mapUnmodifiableMap2 = Collections.unmodifiableMap(this.zzlt);
        }
        this.zzlt = mapUnmodifiableMap2;
        this.zzfk = true;
    }
    ...
    public V get(Object obj) {
        ...
        if (iZzb >= 0) {
            return (V) this.zzlq.get(iZzb).getValue();
        }
    ...
    public final V put(K k, V v) {
        ...
        int iZzb = zzb(k);
        if (iZzb >= 0) {
            return (V) this.zzlq.get(iZzb).setValue(v);
        }
        zzcx();
        ...
        int i2 = this.zzlp;
        if (size == i2) {
            zzdk zzdkVarRemove = this.zzlq.remove(i2 - 1);
    ...
    private final int zzb(K k) {
        ...
        if (size >= 0) {
            int iCompareTo = k.compareTo((Comparable) this.zzlq.get(size).getKey());
            if (iCompareTo > 0) {
                return -(size + 2);
            }
            if (iCompareTo == 0) {
        ...
        while (i <= size) {
            int i2 = (i + size) / 2;
            int iCompareTo2 = k.compareTo((Comparable) this.zzlq.get(i2).getKey());
            if (iCompareTo2 < 0) {
                size = i2 - 1;
            } else {
                if (iCompareTo2 <= 0) {
    ...
    public int hashCode() {
        ...
        int iHashCode = 0;
        for (int i = 0; i < iZzcu; i++) {
            iHashCode += this.zzlq.get(i).hashCode();
        }
        return this.zzlr.size() > 0 ? iHashCode + this.zzlr.hashCode() : iHashCode;
    }

    /* synthetic */ zzdb(int i, zzde zzdeVar) {
        this(i);
    }
}
