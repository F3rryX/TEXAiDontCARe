...
public final class zzar extends zzaw implements Place {
    ...
    public final Locale getLocale() {
        String strZzb = zzb("place_locale_language", "");
        if (!TextUtils.isEmpty(strZzb)) {
            return new Locale(strZzb, zzb("place_locale_country", ""));
        }
        String strZzb2 = zzb("place_locale", "");
        if (!TextUtils.isEmpty(strZzb2)) {
            return new Locale(strZzb2);
        }
    ...
    public final Uri getWebsiteUri() {
        String strZzb = zzb("place_website_uri", (String) null);
        if (strZzb == null) {
            return null;
        }
    ...
    public final /* synthetic */ Place freeze() {
        PlaceEntity placeEntityZzj = new PlaceEntity.zzb().zzd(getAddress().toString()).zzd(zzl()).zzb(getId()).zzb((!hasColumn("place_is_permanently_closed") || hasNull("place_is_permanently_closed")) ? false : getBoolean("place_is_permanently_closed")).zzb(getLatLng()).zzb(zzb("place_level_number", 0.0f)).zzc(getName().toString()).zze(getPhoneNumber().toString()).zzc(getPriceLevel()).zzc(getRating()).zzc(getPlaceTypes()).zzb(getViewport()).zzb(getWebsiteUri()).zzb((zzal) zzb("place_opening_hours", zzal.CREATOR)).zzb(this.zzcf).zzf(zzb("place_adr_address", "")).zzj();
        placeEntityZzj.setLocale(getLocale());
        return placeEntityZzj;
    }
}
