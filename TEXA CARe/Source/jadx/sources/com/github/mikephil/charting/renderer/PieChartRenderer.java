            ...
            if (Math.abs(iPieDataSet2.getEntryForIndex(i8).getY()) > Utils.FLOAT_EPSILON && (!pieChartRenderer.mChart.needsHighlight(i8) || z2)) {
                ...
                if (!z) {
                    ...
                    if (z3) {
                        ...
                        float fCalculateMinimumRadiusForSpacedSlice = calculateMinimumRadiusForSpacedSlice(centerCircleBox, radius, f9 * phaseY, fCos2, fSin2, f11, f7);
                        if (fCalculateMinimumRadiusForSpacedSlice < 0.0f) {
                            fCalculateMinimumRadiusForSpacedSlice = -fCalculateMinimumRadiusForSpacedSlice;
                        }
                        holeRadius = Math.max(f3, fCalculateMinimumRadiusForSpacedSlice);
                    } else {
                        rectF4 = rectF5;
                        f3 = holeRadius;
                        f4 = radius;
                    ...
                    if (f12 >= 360.0f && f7 % 360.0f <= Utils.FLOAT_EPSILON) {
                        ...
                        if (z2) {
                            ...
                            float fCos3 = mPPointF2.x + (((float) Math.cos(d3)) * f19);
                            float fSin3 = mPPointF2.y + (f19 * ((float) Math.sin(d3)));
                            rectF2 = rectF4;
                ...
                if (f5 % f6 <= Utils.FLOAT_EPSILON) {
                    ...
                    rectF3 = rectF2;
                    float fCalculateMinimumRadiusForSpacedSlice2 = calculateMinimumRadiusForSpacedSlice(mPPointF, f4, f9 * phaseY, fCos2, fSin2, f11, f5);
                    double d5 = f20 * 0.017453292f;
                    pieChartRenderer.mPathBuffer.lineTo(mPPointF.x + (((float) Math.cos(d5)) * fCalculateMinimumRadiusForSpacedSlice2), mPPointF.y + (fCalculateMinimumRadiusForSpacedSlice2 * ((float) Math.sin(d5))));
                } else {
                    rectF3 = rectF2;
            ...
            if (zIsDrawValuesEnabled || zIsDrawEntryLabelsEnabled) {
                ...
                while (i5 < entryCount) {
                    ...
                    if (z2 || z3) {
                        ...
                        float fAbs = iPieDataSet2.isValueLineVariableLength() ? valueLinePart2Length * f15 * ((float) Math.abs(Math.sin(d))) : valueLinePart2Length * f15;
                        MPPointF mPPointF9 = mPPointF7;
                        float f22 = (f6 * fCos) + mPPointF9.x;
                        float f23 = (f6 * fSin) + mPPointF9.y;
                        float f24 = (valueLinePart1Length + 1.0f) * f15;
                        f7 = radius;
                        float f25 = (f24 * fCos) + mPPointF9.x;
                        f8 = fSin;
                        float f26 = mPPointF9.y + (f24 * fSin);
                        z = z4;
                        f9 = fCos;
                        ...
                        if (z2 && z3) {
                            ...
                            if (i5 >= pieData.getEntryCount() || label == null) {
                                ...
                                canvas3 = canvas;
                                str = label;
                                drawEntryLabel(canvas3, str, f11, f26 + fCalcTextHeight);
                            }
                        } else {
                            canvas3 = canvas;
                            str = label;
                            if (z2) {
                                if (i5 < pieData.getEntryCount() && str != null) {
                                    drawEntryLabel(canvas3, str, f11, f26 + (fCalcTextHeight / 2.0f));
                                }
                            } else if (z3) {
                                drawValue(canvas, pieLabel, f11, f26 + (fCalcTextHeight / 2.0f), iPieDataSet.getValueTextColor(i5));
                            }
                        }
                    } else {
                        valuePosition = yValuePosition;
                    ...
                    if (z || z5) {
                        ...
                        if (z && z5) {
                            ...
                            if (i5 < pieData.getEntryCount() && str != null) {
                                drawEntryLabel(canvas3, str, f28, f29 + fCalcTextHeight);
                            }
                        } else {
                            if (z) {
                                if (i5 < pieData.getEntryCount() && str != null) {
                                    drawEntryLabel(canvas3, str, f28, f29 + (fCalcTextHeight / 2.0f));
                                }
                            } else if (z5) {
