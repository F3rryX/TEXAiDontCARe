...
public abstract class PieRadarChartBase<T extends ChartData<? extends IDataSet<? extends Entry>>> extends Chart<T> {
    ...
    /* JADX WARN: Type inference failed for: r5v7, types: [com.github.mikephil.charting.components.XAxis, lombok.launch.PatchFixesHider$Delegate] */
    @Override // com.github.mikephil.charting.charts.Chart
    /*
    ...
    */
    public void calculateOffsets() {
        ...
        float f;
        float f2;
        float requiredBaseOffset3 = 0.0f;
        if (this.mLegend == null || !this.mLegend.isEnabled() || this.mLegend.isDrawInsideEnabled()) {
            ...
            if (i == 1) {
                ...
                if (i2 != 1) {
                    if (i2 != 2) {
                        if (i2 == 3) {
                            ...
                            if (i3 == 1) {
                                ...
                                fConvertDpToPixel = 0.0f;
                                f = 0.0f;
                                requiredBaseOffset3 += getRequiredBaseOffset();
                                float requiredBaseOffset4 = fConvertDpToPixel + getRequiredBaseOffset();
                                requiredBaseOffset2 = f2 + getRequiredBaseOffset();
                                requiredBaseOffset = f + getRequiredBaseOffset();
                                r0 = requiredBaseOffset4;
                            } else if (i3 == 2) {
                                fMin = Math.min(this.mLegend.mNeededHeight, this.mViewPortHandler.getChartHeight() * this.mLegend.getMaxSizePercent());
                                f = fMin;
                                fConvertDpToPixel = 0.0f;
                                f2 = 0.0f;
                                requiredBaseOffset3 += getRequiredBaseOffset();
                                float requiredBaseOffset42 = fConvertDpToPixel + getRequiredBaseOffset();
                                requiredBaseOffset2 = f2 + getRequiredBaseOffset();
                                requiredBaseOffset = f + getRequiredBaseOffset();
                                r0 = requiredBaseOffset42;
                            }
                        }
                    }
                    f = 0.0f;
                    f2 = 0.0f;
                    requiredBaseOffset3 += getRequiredBaseOffset();
                    float requiredBaseOffset422 = fConvertDpToPixel + getRequiredBaseOffset();
                    requiredBaseOffset2 = f2 + getRequiredBaseOffset();
                    requiredBaseOffset = f + getRequiredBaseOffset();
                    r0 = requiredBaseOffset422;
                } else {
                    requiredBaseOffset3 = fConvertDpToPixel;
                }
                fConvertDpToPixel = 0.0f;
                f = 0.0f;
                f2 = 0.0f;
                requiredBaseOffset3 += getRequiredBaseOffset();
                float requiredBaseOffset4222 = fConvertDpToPixel + getRequiredBaseOffset();
                requiredBaseOffset2 = f2 + getRequiredBaseOffset();
                requiredBaseOffset = f + getRequiredBaseOffset();
                r0 = requiredBaseOffset4222;
            } else {
                if (i == 2 && (this.mLegend.getVerticalAlignment() == Legend.LegendVerticalAlignment.TOP || this.mLegend.getVerticalAlignment() == Legend.LegendVerticalAlignment.BOTTOM)) {
                    ...
                    f = 0.0f;
                    requiredBaseOffset3 += getRequiredBaseOffset();
                    float requiredBaseOffset42222 = fConvertDpToPixel + getRequiredBaseOffset();
                    requiredBaseOffset2 = f2 + getRequiredBaseOffset();
                    requiredBaseOffset = f + getRequiredBaseOffset();
                    r0 = requiredBaseOffset42222;
                }
                fConvertDpToPixel = 0.0f;
                f = 0.0f;
                f2 = 0.0f;
                requiredBaseOffset3 += getRequiredBaseOffset();
                float requiredBaseOffset422222 = fConvertDpToPixel + getRequiredBaseOffset();
                requiredBaseOffset2 = f2 + getRequiredBaseOffset();
                requiredBaseOffset = f + getRequiredBaseOffset();
                r0 = requiredBaseOffset422222;
            }
        }
        float fConvertDpToPixel3 = Utils.convertDpToPixel(this.mMinOffset);
        if (this instanceof RadarChart) {
            ?? xAxis = getXAxis();
            if (xAxis.isEnabled() && xAxis.handleDelegateForType(r0)) {
        ...
        float extraRightOffset = r0 + getExtraRightOffset();
        float extraBottomOffset = requiredBaseOffset + getExtraBottomOffset();
        float fMax = Math.max(fConvertDpToPixel3, requiredBaseOffset3 + getExtraLeftOffset());
