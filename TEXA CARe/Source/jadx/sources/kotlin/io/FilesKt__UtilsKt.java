...
class FilesKt__UtilsKt extends FilesKt__FileTreeWalkKt {
    ...
    public static final String getNameWithoutExtension(File nameWithoutExtension) {
        ...
        Intrinsics.checkNotNullExpressionValue(name, "name");
        return StringsKt.substringBeforeLast$default(name, Version.SEPARATOR, (String) null, 2, (Object) null);
    }
    ...
    */
    public static final boolean copyRecursively(File copyRecursively, File target, boolean z, final Function2<? super File, ? super IOException, ? extends OnErrorAction> onError) {
        ...
        try {
            for (File file : FilesKt.walkTopDown(copyRecursively).onFail(new Function2<File, IOException, Unit>() { // from class: kotlin.io.FilesKt__UtilsKt.copyRecursively.2
                {
                    super(2);
                }
                ...
                if (!file.exists()) {
                    ...
                    if (file2.exists() && (!file.isDirectory() || !file2.isDirectory())) {
                        if (z) {
                            if (file2.isDirectory()) {
                                z2 = FilesKt.deleteRecursively(file2) ? false : true;
                            } else if (!file2.delete()) {
                            }
                            if (!z2) {
                                if (onError.invoke(file2, new FileAlreadyExistsException(file, file2, "The destination file already exists.")) == OnErrorAction.TERMINATE) {
                                    return false;
                                }
                            }
                        } else if (!z2) {
                    ...
                    if (file.isDirectory()) {
                        file2.mkdirs();
                    } else if (FilesKt.copyTo$default(file, file2, z, 0, 4, null).length() != file.length() && onError.invoke(file, new IOException("Source file wasn't copied completely, length of destination file differs.")) == OnErrorAction.TERMINATE) {
                        return false;
                    }
                }
            }
    ...
    private static final List<File> normalize$FilesKt__UtilsKt(List<? extends File> list) {
        ...
        for (File file : list) {
            ...
            if (name != null) {
                ...
                if (iHashCode == 46) {
                    if (name.equals(Version.SEPARATOR)) {
                    }
