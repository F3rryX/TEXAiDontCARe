...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m2102plusAssignLRDsOJo(double duration) {
        long j;
        double dM2085toDoubleimpl = Duration.m2085toDoubleimpl(duration, getUnit());
        long j2 = (long) dM2085toDoubleimpl;
        if (j2 != Long.MIN_VALUE && j2 != LongCompanionObject.MAX_VALUE) {
            long j3 = this.reading;
            j = j3 + j2;
            if ((j2 ^ j3) >= 0 && (j3 ^ j) < 0) {
                m2101overflowLRDsOJo(duration);
            }
        } else {
            double d = this.reading + dM2085toDoubleimpl;
    ...
    private final void m2101overflowLRDsOJo(double duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + "ns is advanced by " + Duration.m2091toStringimpl(duration) + '.');
    }
}
