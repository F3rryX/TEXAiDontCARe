...
/* JADX INFO: loaded from: com.texa.care.apk:lombok/javac/apt/LombokProcessor.SCL.lombok */
...
public class LombokProcessor extends AbstractProcessor {
    ...
    private void forceNewRound(String randomModuleName, JavacFiler filer) {
        if (!filer.newFiles()) {
            try {
                ...
                this.dummyCount = i + 1;
                generatedSourceNames.add(sbAppend.append(i).toString());
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private String getModuleNameFor(Element element) {
        while (element != null) {
            if (element.getKind().name().equals("MODULE")) {
                return ModuleNameOracle.getModuleName(element);
            }
    ...
    /* JADX INFO: loaded from: com.texa.care.apk:lombok/javac/apt/LombokProcessor$ModuleNameOracle.SCL.lombok */
    private static class ModuleNameOracle {
        private ModuleNameOracle() {
        }
