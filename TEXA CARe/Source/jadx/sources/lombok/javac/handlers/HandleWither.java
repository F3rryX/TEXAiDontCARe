...
/* JADX INFO: loaded from: com.texa.care.apk:lombok/javac/handlers/HandleWither.SCL.lombok */
public class HandleWither extends JavacAnnotationHandler<Wither> {
    ...
    public void createWitherForField(AccessLevel level, JavacNode fieldNode, JavacNode source, boolean strictMode, List<JCTree.JCAnnotation> onMethod, List<JCTree.JCAnnotation> onParam) {
        JavacNode typeNode = fieldNode.up();
        boolean makeAbstract = (typeNode == null || typeNode.getKind() != AST.Kind.TYPE || (typeNode.get().mods.flags & RealWebSocket.DEFAULT_MINIMUM_DEFLATE_SIZE) == 0) ? false : true;
    ...
    public JCTree.JCMethodDecl createWither(long access, JavacNode field, JavacTreeMaker maker, JavacNode source, List<JCTree.JCAnnotation> onMethod, List<JCTree.JCAnnotation> onParam, boolean makeAbstract) {
        ...
        if (makeAbstract) {
            access |= RealWebSocket.DEFAULT_MINIMUM_DEFLATE_SIZE;
        }
