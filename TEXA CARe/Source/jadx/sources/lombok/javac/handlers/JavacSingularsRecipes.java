...
/* JADX INFO: loaded from: com.texa.care.apk:lombok/javac/handlers/JavacSingularsRecipes.SCL.lombok */
public class JavacSingularsRecipes {
    ...
    /* JADX INFO: loaded from: com.texa.care.apk:lombok/javac/handlers/JavacSingularsRecipes$ExpressionMaker.SCL.lombok */
    public interface ExpressionMaker {
        JCTree.JCExpression make();
    }

    /* JADX INFO: loaded from: com.texa.care.apk:lombok/javac/handlers/JavacSingularsRecipes$StatementMaker.SCL.lombok */
    ...
    /* JADX INFO: loaded from: com.texa.care.apk:lombok/javac/handlers/JavacSingularsRecipes$SingularData.SCL.lombok */
    public static final class SingularData {
        ...
        public String getTargetSimpleType() {
            int idx = this.targetFqn.lastIndexOf(Version.SEPARATOR);
            return idx == -1 ? this.targetFqn : this.targetFqn.substring(idx + 1);
        }
    }

    /* JADX INFO: loaded from: com.texa.care.apk:lombok/javac/handlers/JavacSingularsRecipes$JavacSingularizer.SCL.lombok */
    public static abstract class JavacSingularizer {
        ...
        public abstract void generateMethods(SingularData singularData, boolean z, JavacNode javacNode, JCTree jCTree, boolean z2, ExpressionMaker expressionMaker, StatementMaker statementMaker);
        ...
        protected abstract List<JCTree.JCVariableDecl> generateSingularMethodParameters(JavacTreeMaker javacTreeMaker, SingularData singularData, JavacNode javacNode, JCTree jCTree);
        ...
        protected JCTree.JCModifiers makeMods(JavacTreeMaker maker, JavacNode node, boolean deprecate) {
            return deprecate ? maker.Modifiers(1L, List.of(maker.Annotation(JavacHandlerUtil.genJavaLangTypeRef(node, "Deprecated"), List.nil()))) : maker.Modifiers(1L);
        }
        ...
        public void generateMethods(SingularData data, boolean deprecate, final JavacNode builderType, JCTree source, boolean fluent, final boolean chain) {
            ...
            ExpressionMaker returnTypeMaker = new ExpressionMaker() { // from class: lombok.javac.handlers.JavacSingularsRecipes.JavacSingularizer.1
                ...
                public JCTree.JCExpression make() {
                    if (chain) {
                        return JavacHandlerUtil.cloneSelfType(builderType);
                    }
                    return maker.Type(Javac.createVoidType(builderType.getSymbolTable(), Javac.CTC_VOID));
                }
            };
            StatementMaker returnStatementMaker = new StatementMaker() { // from class: lombok.javac.handlers.JavacSingularsRecipes.JavacSingularizer.2
                ...
                public JCTree.JCStatement make() {
                    if (chain) {
                        return maker.Return(maker.Ident(builderType.toName("this")));
                    }
                    return null;
                }
            };
        ...
        private void finishAndInjectMethod(JavacTreeMaker maker, JCTree.JCExpression returnType, JCTree.JCStatement returnStatement, SingularData data, JavacNode builderType, JCTree source, boolean deprecate, ListBuffer<JCTree.JCStatement> statements, Name methodName, List<JCTree.JCVariableDecl> jcVariableDecls) {
        ...
        private void generateClearMethod(boolean deprecate, JavacTreeMaker maker, JCTree.JCExpression returnType, JCTree.JCStatement returnStatement, SingularData data, JavacNode builderType, JCTree source) {
            ...
            finishAndInjectMethod(maker, returnType, returnStatement, data, builderType, source, deprecate, statements, methodName, List.nil());
        }

        private void generateSingularMethod(boolean deprecate, JavacTreeMaker maker, JCTree.JCExpression returnType, JCTree.JCStatement returnStatement, SingularData data, JavacNode builderType, JCTree source, boolean fluent) {
            ListBuffer<JCTree.JCStatement> statements = generateSingularMethodStatements(maker, data, builderType, source);
            List<JCTree.JCVariableDecl> params = generateSingularMethodParameters(maker, data, builderType, source);
            Name name = data.getSingularName();
            if (!fluent) {
                name = builderType.toName(HandlerUtil.buildAccessorName(getAddMethodName(), name.toString()));
            }
        ...
        protected JCTree.JCVariableDecl generateSingularMethodParameter(int typeIndex, JavacTreeMaker maker, SingularData data, JavacNode builderType, JCTree source, Name name) {
        ...
        protected JCTree.JCStatement generateSingularMethodAddStatement(JavacTreeMaker maker, JavacNode builderType, Name argumentName, String builderFieldName) {
            ...
            return maker.Exec(maker.Apply(List.nil(), thisDotFieldDotAdd, List.of(maker.Ident(argumentName))));
        }

        private void generatePluralMethod(boolean deprecate, JavacTreeMaker maker, JCTree.JCExpression returnType, JCTree.JCStatement returnStatement, SingularData data, JavacNode builderType, JCTree source, boolean fluent) {
            ListBuffer<JCTree.JCStatement> statements = generatePluralMethodStatements(maker, data, builderType, source);
            Name name = data.getPluralName();
            if (!fluent) {
                name = builderType.toName(HandlerUtil.buildAccessorName(getAddMethodName() + "All", name.toString()));
            }
            ...
            JCTree.JCVariableDecl param = maker.VarDef(maker.Modifiers(paramFlags), data.getPluralName(), paramType2, null);
            statements.prepend(createConstructBuilderVarIfNeeded(maker, data, builderType, source));
        ...
        protected ListBuffer<JCTree.JCStatement> generatePluralMethodStatements(JavacTreeMaker maker, SingularData data, JavacNode builderType, JCTree source) {
            JCTree.JCExpression thisDotFieldDotAdd = JavacHandlerUtil.chainDots(builderType, "this", data.getPluralName().toString(), getAddMethodName() + "All");
            return new ListBuffer().append(maker.Exec(maker.Apply(List.nil(), thisDotFieldDotAdd, List.of(maker.Ident(data.getPluralName())))));
        }
        ...
        protected JCTree.JCExpression getSize(JavacTreeMaker maker, JavacNode builderType, Name name, boolean nullGuard, boolean parens, String builderVariable) {
            ...
            JCTree.JCMethodInvocation jCMethodInvocationApply = maker.Apply(List.nil(), maker.Select(maker.Select(maker.Ident(thisName), name), builderType.toName("size")), List.nil());
            if (nullGuard) {
