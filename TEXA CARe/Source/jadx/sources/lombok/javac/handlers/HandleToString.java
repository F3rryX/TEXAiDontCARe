...
/* JADX INFO: loaded from: com.texa.care.apk:lombok/javac/handlers/HandleToString.SCL.lombok */
public class HandleToString extends JavacAnnotationHandler<ToString> {
    ...
    static JCTree.JCMethodDecl createToString(JavacNode typeNode, Collection<InclusionExclusionUtils.Included<JavacNode, ToString.Include>> members, boolean includeNames, boolean callSuper, HandlerUtil.FieldAccess fieldAccess, JCTree source) {
        ...
        if (!isEnum) {
            ...
            current = maker.Binary(Javac.CTC_PLUS, maker.Literal(typeName + Version.SEPARATOR), maker.Apply(com.sun.tools.javac.util.List.nil(), maker.Select(maker.Ident(typeNode.toName("this")), typeNode.toName(AppMeasurementSdk.ConditionalUserProperty.NAME)), com.sun.tools.javac.util.List.nil()));
            if (!prefix.isEmpty()) {
        ...
        for (InclusionExclusionUtils.Included<JavacNode, ToString.Include> member : members) {
            ...
            if (first) {
                ...
                if (includeNames) {
                    ...
                    jCBinaryBinary = maker.Binary(Javac.CTC_PLUS, current, maker.Literal(", " + n + "="));
                } else {
    ...
    public static String getTypeName(JavacNode typeNode) {
        ...
        while (true) {
            ...
            if (upType.getKind() == AST.Kind.TYPE) {
                typeName = upType.get().name.toString() + Version.SEPARATOR + typeName;
