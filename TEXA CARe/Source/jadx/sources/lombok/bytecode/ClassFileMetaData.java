...
/* JADX INFO: loaded from: com.texa.care.apk:lombok/bytecode/ClassFileMetaData.SCL.lombok */
public class ClassFileMetaData {
    ...
    private String decodeString(int pos, int size) {
        ...
        while (pos < end) {
            ...
            if (first < 128) {
                ...
                int x = (first & 31) << 6;
                pos++;
                int y = this.byteCode[pos] & Utf8.REPLACEMENT_BYTE;
                int i3 = length;
                length++;
                result[i3] = (char) (x | y);
            } else {
                int x2 = (first & 15) << 12;
                int pos2 = pos + 1;
                int y2 = (this.byteCode[pos] & Utf8.REPLACEMENT_BYTE) << 6;
                pos = pos2 + 1;
                int z = this.byteCode[pos2] & Utf8.REPLACEMENT_BYTE;
                int i4 = length;
                length++;
    ...
    private long readLong(int index) {
        ...
        return (((long) read32(pos)) << 32) | (((long) read32(pos + 4)) & BodyPartID.bodyIdMax);
    }
    ...
    private void appendAccess(StringBuilder result, int index) {
        int pos = this.offsets[index];
        result.append(getClassName(readValue(pos))).append(Version.SEPARATOR);
