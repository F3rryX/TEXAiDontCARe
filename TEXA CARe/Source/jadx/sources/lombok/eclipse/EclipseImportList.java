...
/* JADX INFO: loaded from: com.texa.care.apk:lombok/eclipse/EclipseImportList.SCL.lombok */
public class EclipseImportList implements ImportList {
    ...
    public boolean hasStarImport(String packageName) {
        ...
        if (this.pkg != null && this.pkg.tokens != null && this.pkg.tokens.length == 0) {
            for (Map.Entry<String, Collection<String>> e : LombokInternalAliasing.IMPLIED_EXTRA_STAR_IMPORTS.entrySet()) {
                if (isEqual(e.getKey(), this.pkg) && e.getValue().contains(packageName)) {
                    return true;
                }
            }
        }
        if (this.imports != null) {
            for (ImportReference imp : this.imports) {
                if ((imp.bits & 131072) != 0 && !imp.isStatic()) {
                    if (isEqual(packageName, imp)) {
                        return true;
                    }
                    for (Map.Entry<String, Collection<String>> e2 : LombokInternalAliasing.IMPLIED_EXTRA_STAR_IMPORTS.entrySet()) {
                        if (isEqual(e2.getKey(), imp) && e2.getValue().contains(packageName)) {
                            return true;
                        }
                    }
                }
    ...
    public Collection<String> applyNameToStarImports(String startsWith, String name) {
        ...
        if (this.pkg != null && this.pkg.tokens != null && this.pkg.tokens.length != 0) {
            ...
            if (startsWith.length() == len) {
                ...
                if (match) {
                    out.add(String.valueOf(Eclipse.toQualifiedName(this.pkg.tokens)) + Version.SEPARATOR + name);
                }
            }
        }
        if (this.imports != null) {
                    ...
                    if (firstToken.length == startsWith.length()) {
                        ...
                        while (true) {
                            if (i2 >= firstToken.length) {
                                String fqn = String.valueOf(Eclipse.toQualifiedName(imp.tokens)) + Version.SEPARATOR + name;
                                if (out.isEmpty()) {
    ...
    public String applyUnqualifiedNameToPackage(String unqualified) {
        return (this.pkg == null || this.pkg.tokens == null || this.pkg.tokens.length == 0) ? unqualified : String.valueOf(Eclipse.toQualifiedName(this.pkg.tokens)) + Version.SEPARATOR + unqualified;
    }
}
