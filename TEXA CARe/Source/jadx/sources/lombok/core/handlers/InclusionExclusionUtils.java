...
/* JADX INFO: loaded from: com.texa.care.apk:lombok/core/handlers/InclusionExclusionUtils.SCL.lombok */
public class InclusionExclusionUtils {
    ...
    /* JADX INFO: loaded from: com.texa.care.apk:lombok/core/handlers/InclusionExclusionUtils$Included.SCL.lombok */
    ...
    private static String innerAnnName(Class<? extends Annotation> type) {
        ...
        while (true) {
            ...
            if (c != null) {
                name = String.valueOf(c.getSimpleName()) + Version.SEPARATOR + name;
        ...
        for (L child : typeNode.down()) {
            ...
            if (markExclude) {
                ...
                if (markInclude != null) {
                    ...
                    members.add(new Included<>(child, annotationValues, false));
                } else if (!onlyExplicitlyIncluded) {
                    if (oldIncludes != null) {
                        if (child.getKind() == AST.Kind.FIELD && oldIncludes.contains(name)) {
                            members.add(new Included<>(child, null, false));
                        }
                        ...
                        if (!name.startsWith("$") && !child.isEnumMember()) {
                            members.add(new Included<>(child, null, true));
                        }
                    }
                }
            }
    ...
    public static <A extends AST<A, L, N>, L extends LombokNode<A, L, N>, N> List<Included<L, ToString.Include>> handleToStringMarking(LombokNode<A, L, N> typeNode, AnnotationValues<ToString> annotation, LombokNode<A, L, N> annotationNode) {
        List<Included<L, ToString.Include>> members = handleIncludeExcludeMarking(ToString.Include.class, AppMeasurementSdk.ConditionalUserProperty.NAME, ToString.Exclude.class, typeNode, annotation, annotationNode, true);
        Collections.sort(members, new Comparator<Included<L, ToString.Include>>() { // from class: lombok.core.handlers.InclusionExclusionUtils.1
            ...
            public int compare(Included<L, ToString.Include> included, Included<L, ToString.Include> included2) {
                int startPos;
                int startPos2;
                int iRank = included.getInc() == null ? 0 : ((ToString.Include) included.getInc()).rank();
                int iRank2 = included2.getInc() == null ? 0 : ((ToString.Include) included2.getInc()).rank();
                if (iRank < iRank2) {
                    return 1;
                }
                if (iRank <= iRank2 && (startPos = ((LombokNode) included.getNode()).getStartPos()) >= (startPos2 = ((LombokNode) included2.getNode()).getStartPos())) {
