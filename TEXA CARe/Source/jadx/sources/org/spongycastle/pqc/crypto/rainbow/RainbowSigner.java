...
public class RainbowSigner implements MessageSigner {
    ...
    */
    public byte[] generateSignature(byte[] bArr) {
        ...
        do {
            try {
                sArrInitSign = initSign(layers, sArrMakeMessageRepresentative);
                i2 = 0;
            ...
            for (i = 0; i < length; i++) {
                ...
                short[] sArrSolveEquation = this.cf.solveEquation(layers[i].plugInVinegars(this.x), sArr);
                if (sArrSolveEquation == null) {
                    throw new Exception("LES is not solveable!");
                }
                for (int i5 = 0; i5 < sArrSolveEquation.length; i5++) {
                    this.x[layers[i].getVi() + i5] = sArrSolveEquation[i5];
                }
                if (!z) {
                    break;
                }
