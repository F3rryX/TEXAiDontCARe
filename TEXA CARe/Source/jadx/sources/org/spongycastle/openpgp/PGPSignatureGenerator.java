...
public class PGPSignatureGenerator {
    ...
    public void update(byte b) {
        if (this.sigType == 1) {
            if (b == 13) {
                byteUpdate(DataManagerParameterID.DRIVING_TIME_LOW_RPM_MAX_THRESHOLD);
                byteUpdate((byte) 10);
            } else if (b == 10) {
                if (this.lastb != 13) {
                    byteUpdate(DataManagerParameterID.DRIVING_TIME_LOW_RPM_MAX_THRESHOLD);
    ...
    public PGPSignature generate() throws PGPException {
        SignatureSubpacket[] signatureSubpacketArrInsertSubpacket;
        SignatureSubpacket[] signatureSubpacketArrInsertSubpacket2;
        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
        if (!packetPresent(this.hashed, 2)) {
            signatureSubpacketArrInsertSubpacket = insertSubpacket(this.hashed, new SignatureCreationTime(false, new Date()));
        } else {
            signatureSubpacketArrInsertSubpacket = this.hashed;
        }
        SignatureSubpacket[] signatureSubpacketArr = signatureSubpacketArrInsertSubpacket;
        if (!packetPresent(this.hashed, 16) && !packetPresent(this.unhashed, 16)) {
            signatureSubpacketArrInsertSubpacket2 = insertSubpacket(this.unhashed, new IssuerKeyID(false, this.contentSigner.getKeyID()));
        } else {
            signatureSubpacketArrInsertSubpacket2 = this.unhashed;
        }
        SignatureSubpacket[] signatureSubpacketArr2 = signatureSubpacketArrInsertSubpacket2;
        byte b = (byte) 4;
        try {
            ...
            for (int i = 0; i != signatureSubpacketArr.length; i++) {
                signatureSubpacketArr[i].encode(byteArrayOutputStream2);
            }
            ...
            return new PGPSignature(new SignaturePacket(this.sigType, this.contentSigner.getKeyID(), this.contentSigner.getKeyAlgorithm(), this.contentSigner.getHashAlgorithm(), signatureSubpacketArr, signatureSubpacketArr2, new byte[]{digest[0], digest[1]}, mPIntegerArrDsaSigToMpi));
        } catch (IOException e) {
    ...
    public PGPSignature generateCertification(PGPUserAttributeSubpacketVector pGPUserAttributeSubpacketVector, PGPPublicKey pGPPublicKey) throws PGPException {
        ...
        try {
            ...
            updateWithIdData(Commands.DPM_Start, byteArrayOutputStream.toByteArray());
            return generate();
        } catch (IOException e) {
