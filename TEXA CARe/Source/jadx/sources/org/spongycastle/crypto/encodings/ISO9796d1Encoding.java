...
public class ISO9796d1Encoding implements AsymmetricBlockCipher {
    ...
    private static byte[] shadows = {DataManagerParameterID.LED_ALWAYS_ON, 3, 5, 8, 9, 4, 2, 15, 0, DataManagerParameterID.DRIVING_TIME_LOW_RPM_MAX_THRESHOLD, DataManagerParameterID.ACCELERATION_MAX_RANGE, 6, 7, 10, DataManagerParameterID.DRIVING_TIME_LOW_RPM_MIN_THRESHOLD, 1};
    private static byte[] inverse = {8, 15, 6, 1, 5, 2, DataManagerParameterID.ACCELERATION_MAX_RANGE, DataManagerParameterID.DRIVING_TIME_LOW_RPM_MIN_THRESHOLD, 3, 4, DataManagerParameterID.DRIVING_TIME_LOW_RPM_MAX_THRESHOLD, 10, DataManagerParameterID.LED_ALWAYS_ON, 9, 0, 7};
    ...
    private byte[] decodeBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        byte[] bArr2 = shadows;
        bArrConvertOutputDecryptOnly[0] = (byte) (bArr2[bArrConvertOutputDecryptOnly[1] & 15] | (bArr2[(bArrConvertOutputDecryptOnly[1] & 255) >>> 4] << 4));
        int i4 = 0;
        boolean z = false;
        int i5 = 1;
        for (int length = bArrConvertOutputDecryptOnly.length - 1; length >= bArrConvertOutputDecryptOnly.length - (i3 * 2); length -= 2) {
            byte[] bArr3 = shadows;
            int i6 = bArr3[bArrConvertOutputDecryptOnly[length] & 15] | (bArr3[(bArrConvertOutputDecryptOnly[length] & 255) >>> 4] << 4);
            int i7 = length - 1;
            if (((bArrConvertOutputDecryptOnly[i7] ^ i6) & 255) != 0) {
                if (!z) {
                    i5 = (bArrConvertOutputDecryptOnly[i7] ^ i6) & 255;
                    i4 = i7;
                    z = true;
                } else {
        ...
        bArrConvertOutputDecryptOnly[i4] = 0;
        int length2 = (bArrConvertOutputDecryptOnly.length - i4) / 2;
        byte[] bArr4 = new byte[length2];
        for (int i8 = 0; i8 < length2; i8++) {
            bArr4[i8] = bArrConvertOutputDecryptOnly[(i8 * 2) + i4 + 1];
        }
        this.padBits = i5 - 1;
        return bArr4;
    }
