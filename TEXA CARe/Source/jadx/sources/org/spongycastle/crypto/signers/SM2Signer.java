...
public class SM2Signer implements DSA, ECConstants {
    ...
    public BigInteger[] generateSignature(byte[] bArr) {
        ...
        while (true) {
            ...
            if (!bigIntegerMod.equals(ZERO) && !bigIntegerMod.add(bigIntegerNextK).equals(n)) {
                BigInteger bigIntegerMod2 = d.add(ONE).modInverse(n).multiply(bigIntegerNextK.subtract(bigIntegerMod.multiply(d)).mod(n)).mod(n);
    ...
    public boolean verifySignature(byte[] bArr, BigInteger bigInteger, BigInteger bigInteger2) {
        ...
        return bigInteger.equals(bigIntegerCalculateE.add(this.ecParams.getG().multiply(bigInteger2).add(q.multiply(bigIntegerMod)).normalize().getAffineXCoord().toBigInteger()).mod(n));
    }
