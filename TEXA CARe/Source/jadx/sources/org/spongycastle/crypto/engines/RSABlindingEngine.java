...
public class RSABlindingEngine implements AsymmetricBlockCipher {
    ...
    public byte[] processBlock(byte[] bArr, int i, int i2) {
        BigInteger bigIntegerUnblindMessage;
        BigInteger bigIntegerConvertInput = this.core.convertInput(bArr, i, i2);
        if (this.forEncryption) {
            bigIntegerUnblindMessage = blindMessage(bigIntegerConvertInput);
        } else {
            bigIntegerUnblindMessage = unblindMessage(bigIntegerConvertInput);
        }
        return this.core.convertOutput(bigIntegerUnblindMessage);
    }
