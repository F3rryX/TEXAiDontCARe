...
/* JADX INFO: loaded from: com.texa.care.apk:org/objectweb/asm/Frame.SCL.lombok */
class Frame {
            ...
            case '[':
                ...
                switch (buffer.charAt(elementDescriptorOffset)) {
                    ...
                    case 'L':
                        ...
                        typeValue = REFERENCE_KIND | symbolTable.addType(internalName2);
                        break;
                    case 'S':
                        typeValue = SHORT;
                        break;
    ...
    final void setInputFrameFromDescriptor(SymbolTable symbolTable, int access, String descriptor, int maxLocals) {
        ...
        if ((access & 8) == 0) {
            if ((access & 262144) == 0) {
                inputLocalIndex = 0 + 1;
                this.inputLocals[0] = REFERENCE_KIND | symbolTable.addType(symbolTable.getClassName());
            } else {
                inputLocalIndex = 0 + 1;
    ...
    private int getInitializedType(SymbolTable symbolTable, int abstractType) {
        if (abstractType == UNINITIALIZED_THIS || (abstractType & ViewCompat.MEASURED_STATE_MASK) == UNINITIALIZED_KIND) {
            ...
            case 58:
                ...
                if (arg > 0) {
                    ...
                    if (previousLocalType == LONG || previousLocalType == DOUBLE) {
                        ...
                        if ((previousLocalType & KIND_MASK) == LOCAL_KIND || (previousLocalType & KIND_MASK) == STACK_KIND) {
            ...
            case 57:
                ...
                if (arg > 0) {
                    ...
                    if (previousLocalType2 == LONG || previousLocalType2 == DOUBLE) {
                        ...
                        if ((previousLocalType2 & KIND_MASK) == LOCAL_KIND || (previousLocalType2 & KIND_MASK) == STACK_KIND) {
    ...
    private static boolean merge(SymbolTable symbolTable, int sourceType, int[] dstTypes, int dstIndex) {
        ...
        if ((dstType & (-268435456)) != 0 || (dstType & KIND_MASK) == REFERENCE_KIND) {
            if (srcType == NULL) {
                return false;
            }
            if ((srcType & ViewCompat.MEASURED_STATE_MASK) == (dstType & ViewCompat.MEASURED_STATE_MASK)) {
                if ((dstType & KIND_MASK) == REFERENCE_KIND) {
                    mergedType = (srcType & (-268435456)) | REFERENCE_KIND | symbolTable.addMergedType(srcType & VALUE_MASK, dstType & VALUE_MASK);
                } else {
                    int mergedDim = (-268435456) + (srcType & (-268435456));
                    mergedType = mergedDim | REFERENCE_KIND | symbolTable.addType("java/lang/Object");
                }
            } else if ((srcType & (-268435456)) != 0 || (srcType & KIND_MASK) == REFERENCE_KIND) {
            ...
            mergedType = ((srcType & (-268435456)) != 0 || (srcType & KIND_MASK) == REFERENCE_KIND) ? srcType : 16777216;
        } else {
            mergedType = 16777216;
        }
    ...
    static void putAbstractType(SymbolTable symbolTable, int abstractType, ByteVector output) {
        ...
        if ((abstractType & KIND_MASK) == REFERENCE_KIND) {
            typeDescriptor.append(Matrix.MATRIX_TYPE_RANDOM_LT).append(symbolTable.getType(abstractType & VALUE_MASK).value).append(';');
        } else {
            switch (abstractType & VALUE_MASK) {
                ...
                default:
                    throw new AssertionError();
                case 9:
                    typeDescriptor.append(Matrix.MATRIX_TYPE_ZERO);
                    break;
                case 10:
