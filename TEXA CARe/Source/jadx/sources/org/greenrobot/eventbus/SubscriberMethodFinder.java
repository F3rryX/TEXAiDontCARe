        ...
        for (Method method : methods) {
            ...
            if ((modifiers & 1) != 0 && (modifiers & MODIFIERS_IGNORE) == 0) {
                ...
                if (parameterTypes.length == 1) {
                    ...
                    throw new EventBusException("@Subscribe method " + (method.getDeclaringClass().getName() + Version.SEPARATOR + method.getName()) + "must have exactly 1 parameter but has " + parameterTypes.length);
                }
            } else if (this.strictMethodVerification && method.isAnnotationPresent(Subscribe.class)) {
                throw new EventBusException((method.getDeclaringClass().getName() + Version.SEPARATOR + method.getName()) + " is a illegal @Subscribe method: must be public, non-static, and non-abstract");
            }
        }
    }
