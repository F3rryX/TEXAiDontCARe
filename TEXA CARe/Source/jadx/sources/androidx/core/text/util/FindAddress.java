...
class FindAddress {
    ...
    private static class ZipRange {
        ...
        boolean matches(String zipCode) {
            int i = Integer.parseInt(zipCode.substring(0, 2));
    ...
    public static MatchResult matchHouseNumber(String content, int offset) {
        ...
        Matcher matcherRegion = sHouseNumberRe.matcher(content).region(offset, content.length());
    ...
    public static MatchResult matchState(String content, int offset) {
        ...
        Matcher matcherRegion = sStateRe.matcher(content).region(offset, content.length());
        if (matcherRegion.lookingAt()) {
    ...
    private static boolean isValidZipCode(String zipCode, MatchResult stateMatch) {
        ...
        int iGroupCount = stateMatch.groupCount();
        while (true) {
            if (iGroupCount <= 0) {
                break;
            }
            int i = iGroupCount - 1;
            if (stateMatch.group(iGroupCount) != null) {
                iGroupCount = i;
                break;
            }
            iGroupCount = i;
        }
        return sZipCodeRe.matcher(zipCode).matches() && sStateZipCodeRanges[iGroupCount].matches(zipCode);
    }
        ...
        while (true) {
            if (iEnd < content.length()) {
                ...
                while (iEnd < matcher.start()) {
                    int i4 = iEnd + 1;
                    if (NL.indexOf(content.charAt(iEnd)) != -1) {
                        i++;
                    }
                    iEnd = i4;
                }
                ...
                if (matchHouseNumber(content, iEnd) == null) {
                    if (!isValidLocationName(matcher.group(0))) {
                        if (i2 == 5 && !z2) {
                            iEnd = matcher.end();
                            break;
                        }
                        if (z2 && i2 > 4 && (matchResultMatchState = matchState(content, iEnd)) != null) {
    ...
    static String findAddress(String content) {
        ...
        while (matcher.find(iEnd)) {
            if (checkHouseNumber(matcher.group(0))) {
                ...
                int iAttemptMatch = attemptMatch(content, matcher);
                if (iAttemptMatch > 0) {
                    return content.substring(iStart, iAttemptMatch);
                }
                iEnd = -iAttemptMatch;
            } else {
                iEnd = matcher.end();
